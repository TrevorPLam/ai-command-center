...
- [ ] **POL‑005G**: Motion Compliance Checklist  
  - Every `motion.div` inside `AnimatePresence` has a `key` prop (required for exit animations).
  - ⚠️ **CRITICAL: Every `layoutId` must be wrapped in a module-scoped `LayoutGroup` with a unique `id` prop.** `layoutId` is global across the entire site. Without `LayoutGroup` namespacing, multiple instances of the same component (e.g., two Kanban boards, two tab rows) will have colliding `layoutId` values, causing broken animations. Audit all components using `layoutId` and verify each is wrapped in `<LayoutGroup id="module-name">`. See FND-013K for the full audit procedure.
  - `useReducedMotion()` is checked at the top of every animated component.
  - No animation exceeds 150ms on a Quiet‑tier element (tooltips, reveals).
  - No easing functions other than spring physics appear on Alive‑tier elements (core nav, state changes).

...
**DEPRECATED:** This file is archived. The canonical source for all ADRs, decisions, and questions is now the **Unified Decision Log** at `01-PLAN-DECISIONS.md`. Please refer to that file for the complete and up-to-date record.

---

Maps all ADRs to domain + summary. Check before implementing.

**Statuses:** active/superseded/updated  
**Domains:** Platform/Data/AI-Core/Frontend/Security/Business

## ADR Index (by ID)

|ADR ID|Domain|Summary|Status|Related DEC|
|---|---|---|---|---|
...
|ADR_004|AI-Core|LiteLLM as AI proxy/gateway|active|--|
|ADR_005|Platform|Y-Sweet + Yjs for real-time collab (self-host mandatory)|updated (ADR_083)|--|
|ADR_006|Frontend|LiveKit for real-time comm (v2.0 only per ADR_115)|updated (ADR_115)|--|
|ADR_007|Platform|Edge Functions for webhooks (now Serverless/FastAPI per ADR_099)|updated (ADR_099)|--|
...
|ADR_012|AI-Core|Embeddings computed only on backend|active|--|
|ADR_014|Frontend|Pin react-big-calendar ^1.19.4 for React 19 compatibility|updated|--|
...
|ADR_019|AI-Core|LiveKit Agents v1 (updated to v2.0 only per ADR_115)|updated|--|
...
|ADR_028|AI-Core|AI evaluation in CI as mandatory gate|active|--|
...
|ADR_036|AI-Core|Contextual Retrieval (Phase 2, cost model updated 50K threshold)|updated (ADR_118)|--|
...
|ADR_067|Security|MCP security model: OAuth, schema allowlist, high-risk elicitation|updated (ADR_096)|--|
|ADR_076|Frontend|Yjs lifecycle: GC on, undo truncation last 5, snapshot version history|active|--|
|ADR_077|Data|Nylas v3 webhooks: upsert-first, 10s timeout, async queue|active|--|
|ADR_079|Data|Offline-first tombstone pattern: `deleted_at` (nullable ms), ULID, idempotency keys|active|--|
|ADR_082|Frontend|Recurrence engine: rschedule + Temporal date adapter, ZonedDateTime mandatory|updated (ADR_109)|--|
...
|ADR_086|AI-Core|Vercel AI SDK v6 for streaming, tool calling, AI Gateway|active|--|
...
|ADR_096|Security|MCPSec L2 mandatory for all MCP servers (replaces ADR_067)|active|--|
...
|ADR_099|Platform|Vercel Serverless (not Edge) for DB-dependent webhooks|active|--|
...
*Updated ADRs listed under primary domain.*

---

## Usage

- **Before implementing**: Scan relevant domain. If ADR exists, read summary + decide if applies.
- **If superseding**: Append row with status `superseded` + link to new ADR/Decision Register.
- **ADR_054 + DEC-2026-04-26-001**: ADR refined in Decision Register. ADR = standard; Decision Register = process.
...
| ID | Feature | Notes |
|---|---|---|
| F006 | Ollama ≥0.21.1: Docker Compose, OpenAI-compatible API, native tool calling | 165K+ stars. Supports Gemma 4, Qwen3.5, Phi‑4‑mini, Llama 4 |
| F007 | llama.cpp backend with ik_llama.cpp fork: BitNet, fused MoE, hybrid GPU/CPU ops | Better CPU & hybrid performance than upstream |
| F008 | Model registry: pull, verify checksum (SHA‑256), register, serve | Gemma 4 E2B (2B active, native tool calling, multimodal) default; Qwen3.5 4B executor |
| F009 | GGUF Q4_K_M quantization: all local models ≤4.5 GB RAM footprint | Human reading speed (~6 TPS) on consumer CPU |
| F010 | LiteLLM proxy integration (≥1.83.7, cosign verified) | Claude Sonnet 4.6 ($3/$15 per 1M tok), Opus 4.7 premium cloud fallback |
...
| F015 | LangGraph Supervisor pattern mapping to FLOWC01 state machine | Centralized supervisor routes to specialized workers; LangMem for cross‑session summaries |
...
| F021 | Calendar MonthView, WeekDayView, AgendaView | @M MotionGuard, @Z TimezoneAware, ARIA grid nav, keyboard shortcuts |
...
| F040 | Subscription tiers: Free (local models only), Pro ($20–30/mo, Sonnet 4.6 cloud), Team (Opus 4.7, fine‑tuning) | Stripe customer portal for self‑service |
...
| ID | Feature | Notes |
|---|---|---|
| P104 | Nylas Email API: read, send, webhooks | Upsert‑first, 10s timeout, async queue. **Cost gate:** evaluate at 50 accounts ($165/mo at ~$3.30/account); if >$200/mo, defer expansion |
| P105 | Email‑triggered conflict detection: reschedule email → agent parses → checks conflicts | Local model (Haiku 4.5 or Gemma 4 E2B) for parsing ($1/$5 per 1M tok cloud). Intent Dispatcher routes to lightweight model |
...
| ID | Feature | Notes |
|---|---|---|
| P110 | Verifier cascade: Phi‑4‑mini‑reasoning (3.8B) as local verifier | Checks: reasoning soundness, schema validity, permission/cost budget. Function calling supported on Phi‑4‑mini |
...
| ID | Feature | Notes |
|---|---|---|
| P209 | Fine‑tuning: Unsloth (GPU, 2× faster, 70% less VRAM) for QLoRA; LoFT CLI for CPU‑only 1–3B models; MLX‑LM for Apple Silicon | Domain‑specific task models, per‑org style adaptation |
